...
export default function FreelanceProfilePage() {
  ...
  useEffect(() => {
    authFetch<FreelanceProfile>(`/profiles/freelances/${params.id}`)
      .then(setProfile)
  ...
  async function contact() {
    setContacting(true);
    try {
      const conversation = await authFetch<{ id: string }>(`/messaging/conversations/salon/${params.id}`, {
        method: "POST",
      });
        ...
        <div>
          <div className="relative aspect-[4/3] w-full overflow-hidden rounded-3xl">
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-noir-chaud via-noir-chaud/20 to-transparent p-5 text-ivoire">
              <p className="text-xs uppercase tracking-widest text-ivoire/70">{profile.villeBase}</p>
              <h1 className="font-serif text-3xl">
                {profile.prenom} {profile.nom}
              </h1>
              {profile.anneesExperience !== null && (
                <p className="mt-1 text-sm text-ivoire/90">{profile.anneesExperience} ans d&apos;expérience</p>
              )}
            </div>
          ...
          {profile.bio && <p className="mt-4 text-sm leading-relaxed text-noir-chaud/80">{profile.bio}</p>}

          <div className="mt-4 flex flex-wrap gap-1.5">
            {profile.specialties.map((s) => (
              <span key={s.name} className="rounded-full bg-noir-chaud/5 px-3 py-1 text-xs text-noir-chaud/70">
                {s.name}
              </span>
            ))}
          </div>
          ...
          <h2 className="mt-8 font-serif text-lg">Tarifs</h2>
          <div className="mt-3 space-y-2">
            {profile.tarifsAffiches.map((t) => (
              <div key={t.prestation} className="flex justify-between rounded-xl bg-white/50 p-3 text-sm">
          ...
          {profile.portfolio.length > 0 && (
            <>
              <h2 className="mt-8 font-serif text-lg">Portfolio</h2>
              <div className="mt-3 grid grid-cols-2 gap-2">
                {profile.portfolio.map((p) => (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img key={p.id} src={p.imageUrl} alt="" className="aspect-square w-full rounded-xl object-cover" />
                ))}
              </div>
            </>
          )}
